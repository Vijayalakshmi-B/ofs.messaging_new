...
public class MessagingContext implements Context {
	...
	public MessagingContext(Hashtable env) {
		if (env == null) {
	...
	public MessagingContext(Hashtable environment, Map bindings) {
		if (environment == null) {
			this.environment = new Hashtable();
		} else {
	...
	public MessagingContext(Hashtable environment, Map bindings, String nameInNamespace) {
		this(environment, bindings);
		this.nameInNamespace = nameInNamespace;
	}

	protected MessagingContext(MessagingContext clone, Hashtable env) {
		this.bindings = clone.bindings;
		this.treeBindings = clone.treeBindings;
		this.environment = new Hashtable(env);
	}

	protected MessagingContext(MessagingContext clone, Hashtable env, String nameInNamespace) {
		this(clone, env);
		this.nameInNamespace = nameInNamespace;
	}
	...
	protected Map internalBind(String name, Object value) throws NamingException {
		...
		if (pos == -1) {
			...
			} else if (!(o instanceof MessagingContext)) {
				throw new NamingException("Something already bound where a subcontext should go");
			}
			MessagingContext readOnlyContext = (MessagingContext) o;
	...
	protected MessagingContext newContext() {
	...
	public Object lookup(String name) throws NamingException {
		...
		if (result instanceof Reference) {
			try {
				...
				throw (NamingException) new NamingException("could not look up : " + name)
						.initCause(e);
			}
		}
		if (result instanceof MessagingContext) {
			String prefix = getNameInNamespace();
			if (prefix.length() > 0) {
				prefix = prefix + SEPARATOR;
			}
			result = new MessagingContext((MessagingContext) result, environment, prefix + name);
		}
		return result;
	}
